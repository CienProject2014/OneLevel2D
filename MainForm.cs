...
namespace OneLevel2D
{
    public partial class MainForm : Form
    {
        ...
        private void AddEvent()
        {
            ...
            Board.DragDrop += (sender, e) =>
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    State.MakeNewImage(name, transformedLocation);
        ...
        private void assetRemoveToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (assetList.SelectedItems.Count > 0)
            {
                for (int i = assetList.SelectedItems.Count - 1; i >= 0; i--)
                {
                    ...
                    string fileDir = GetAssetDirectory(asset);

                    // Document에서 Asset과 관련된 Component들을 지운다. (Component가 사용하는 리소스도 모두 해제)
                    ...
                    if (File.Exists(fileDir))
                    {
                        ...
                        {
                            MessageBox.Show(@"입출력 오류. "+exception.Message);
                        }
                        catch (UnauthorizedAccessException exception)
                        {
                            MessageBox.Show(@"권한이 없는 오류. " + exception.Message);
                        }
                    }
                }
            }
        ...
        private string GetAssetDirectory(Asset asset)
        {
            string dir = null;
            switch (asset.Type)
            {
